function [FeaturesTotal,NodeLab,NodeAtt,NumNodes] = NstepFeature(StepN,G,NodeLab_Exist,NodeAtt_Exist)
...
AdjMat=G.A+1*eye(NumNodes); Deg=AdjMat*ones(NumNodes,1); 
...
if StepN<=5
    ...
    U=U.*U;
    for s=1:StepN
        FeaturesTotal(s,:)=U*(TempD);% each column is an RPF for a node
        TempD=TempD.*ColD;
    end
end
